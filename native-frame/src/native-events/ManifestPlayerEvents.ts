...
export class ManifestPlayerEvents {
  ...
  subscribeEvents() {
    this.emitter =
      Platform.OS === 'android'
        ? DeviceEventEmitter
        : new NativeEventEmitter(NativeModules.ManifestPlayerEvents);
  ...
  onManifestUnauthorized(manifest: string) {
    NativeModules.ManifestPlayerEvents?.onManifestUnauthorized({
      manifest: manifest,
    });
  }
  ...
  onManifestSourceChange(source: string, peerId?: string) {
    NativeModules.ManifestPlayerEvents?.onManifestSourceChange({
      source: source,
      peerId: peerId,
    });
  }
  ...
  onAvailableQualities(availableQualities: string[]) {
    NativeModules.ManifestPlayerEvents?.onAvailableQualities({
